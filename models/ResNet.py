...
__all__ = ['AP3DResNet50', 'AP3DNLResNet50','Baseline','NLResNet50', 'ResNet50TA']
...
class ResNet50TA(nn.Module):
    def __init__(self, num_classes, loss={'xent', 'htri'}, **kwargs):
        ...
        self.attention_conv = nn.Conv2d(self.feat_dim, self.middle_dim,
                                        [8, 4])  # 7,4 cooresponds to 224, 112 input image size
...
def Baseline(num_classes, **kwargs):
    c3d_idx = [[],[],[],[]]
    nl_idx = [[],[],[],[]]

    return ResNet503D(num_classes, AP3D.APP3DC, c3d_idx, nl_idx, **kwargs)

def NLResNet50(num_classes, **kwargs):
    c3d_idx = [[],[],[],[]]
    nl_idx = [[],[1, 3],[1, 3, 5],[]]

    return ResNet503D(num_classes, AP3D.APP3DC, c3d_idx, nl_idx, **kwargs)

def ResnetTA(num_classes, **kwargs):
    pass
    return ResNet50TA(num_classes,  **kwargs)

def ResnetTP(num_classes, **kwargs):
    pass
    return ResNet50TP(num_classes, **kwargs)
